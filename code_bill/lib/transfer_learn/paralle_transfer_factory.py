    ...
    def run(self, p: Param):
        ...
        for fold in range(5):
            ...
            if p.pretrain_model.split('-')[0] == 'roberta':
                model = RoBERTaFinetuner(ep=p,fold=fold,feature_dim=twdata.feature_dim, max_epoch=self.max_epoch)
            elif p.pretrain_model.split('-')[0] == 'bert':
                model = BertMNLIFinetuner(ep=p,fold=fold,feature_dim=twdata.feature_dim, max_epoch=self.max_epoch)
            else:
            ...
            if os.path.isfile(self.ckp_cb.best_model_path):
                logger.info('rm file {self.ckp_cb.best_model_path}')
